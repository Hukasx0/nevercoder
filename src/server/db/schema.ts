 ...
 */
export const createTable = pgTableCreator((name) => `nevercoder_${name}`);
...
export const websites = createTable(
  "website",
  {
    ...
    content: varchar("content", { length: 30720 }).notNull(),
  },
)

export const websitesRelations = relations(websites, ({ one }) => ({
  user: one(users, { fields: [websites.createdById], references: [users.id] }),
}));
